...
const { MONGO_URL_TESTING } = process.env;

const connectMongo = async () => {
    // Connecting to the database
    try {
      await mongoose.connect(MONGO_URL_TESTING!)
...
const parsePick = async () => {
    ...
    const pick: PickSchema | null = await Pick.findById("63264fa50418d49b13101dd3");
    ...
    stringify(data, { header: true, columns: columns }, (err, output) => {
        if (err) throw err;
        fs.writeFile('my2.csv', output, (err) => {
          if (err) throw err;
          console.log('my2.csv saved.');
        });
      });
